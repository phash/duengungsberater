...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function logout() {
    ...
    userEmail.value = null
    isAdminUser.value = false
    // Service-Worker-Cache leeren — verhindert, dass RLS-geschützte API-Antworten
    // des ausgeloggten Users sichtbar bleiben (z.B. wenn anderer User auf gleichem Gerät)
    if (typeof caches !== 'undefined') {
      try {
        const keys = await caches.keys()
        await Promise.all(
          keys.filter((k) => k.includes('api-cache')).map((k) => caches.delete(k)),
        )
      } catch {
        /* SW-Caches optional — fail silent */
      }
    }
